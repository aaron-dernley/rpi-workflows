...
1. **This repo has no extension of its own.** Unlike the sibling
   `@aaronge/rpi-*`/`@aaronge/host-health`/`@aaronge/apt-inventory` repos,
   nothing here is published as a `@aaronge/*` extension — there's no
   `manifest.yaml`, no `extensions/models/`. This repo exists purely to
   host workflows that join data *across* those already-published
   extensions via `swamp extension pull` + `data.latest()`/CEL. New
   cross-cutting workflows belong here; new observation logic belongs in
   its own single-purpose extension repo instead.
2. **README at repo root**, same as the sibling repos, documenting each
   workflow's purpose, schedule, and which pulled models/instances it reads.
